...
def load_data(args):
    ...
    source_loader, n_class = data_loader.load_data_train(
        X_train, Y_train, W_train, P_train, I_train, args.batch_size,
        infinite_data_loader=not args.epoch_based_training, train=True, num_workers=args.num_workers)
    # print("FINISH TRAIN")
    target_train_loader, T_class = data_loader.load_data_train(
        X_train2, Y_train2, W_train2, P_train2, I_train2, args.batch_size,
        infinite_data_loader=not args.epoch_based_training, train=True, num_workers=args.num_workers)
    # print("TT_class",T_class)
    target_test_loader, _ = data_loader.load_data_test(
        X_test, Y_test, W_test, P_test, I_test, args.batch_size, infinite_data_loader=False, train=False,
...
def get_model(cl_weight, myPI, args):
    model = models.TransferNet(args.n_class, myPI=myPI, transfer_loss=args.transfer_loss, base_net=args.backbone,
                               max_iter=args.max_iter, cl_weight=cl_weight, use_bottleneck=args.use_bottleneck).to(
        args.device)
    # print(model)
    return model
...
def get_finetune_scheduler(optimizer, args):
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer,
                                                  lambda x: args.lr * 0.001 * (1. + args.lr_gamma * float(x)) ** (
                                                      -args.lr_decay))
    return scheduler
...
def test(model, target_test_loader, cl_weight, args):
    ...
    correct = 0
    criterion = torch.nn.CrossEntropyLoss(weight=torch.from_numpy(np.array(cl_weight)).float().to(args.device),
                                          size_average=True)
...
def input(model, args):
    path = "../pth/" + str(args.src_domain) + "+" + str(args.tgt_domain) + "/" + str(args.config.split("/")[0]) + \
           "_" + str(args.transfer_loss) + "_" + str(args.method) + "_" + str(args.myweight) + "_" + str(
        args.seed) + "_"
...
def output(model, y_true, y_pred, name, args):
    ...
    path = "../pth/" + str(args.src_domain) + "+" + str(args.tgt_domain) + "/"
    if not os.path.exists(path):
        os.makedirs(path)
    path += str(args.config.split("/")[0]) + "_" + str(args.transfer_loss) + "_" + str(args.method) + "_" + str(args.myweight) + "_" + str(args.seed) + "_"
    if name == 'SAMPLE':
        if best_f1_ma < f1_ma:
            best_f1_ma = f1_ma
        ...
        print("BEST_F1_MI:", best_f1_mi, "BEST_F1_MA:", best_f1_ma, "BEST_ACC_MI:", best_acc_mi, "BEST_F1_MA:",
              best_acc_ma)
    print(confusion_matrix(y_true, y_pred))
    return best_f1_ma


def train(source_loader, target_train_loader, target_test_loader, L_test, model, optimizer, lr_scheduler, cl_weight,
          args):
    ...
    for e in range(1, args.n_epoch + 1):
        ...
        for i in range(n_batch):
            ...
            # (32, 3, 128)
            data_source, label_source, weight_source, person_source, item_source, data_source2, label_source2, weight_source2 = next(
                iter_source)  # .next()
            data_target, _, weight_target, person_target, item_target, data_target2, _, weight_target2 = next(
                iter_target)  # .next()
            if args.method == 'True':
                print("!!!!!!!!!!", data_source.shape, data_source2.shape)
            ...
            clf_loss, transfer_loss, person_loss = model(data_source, data_target, label_source, person_source,
                                                         item_source, weight_source, weight_target, person_target,
                                                         item_target, mu_list, e, args.method)
...
def main():
    ...
    if args.tname == "self":
        ...
        train(source_loader, target_train_loader, target_test_loader, L_test, model, optimizer, scheduler, cl_weight,
              args)
        print("ARG:", args)
